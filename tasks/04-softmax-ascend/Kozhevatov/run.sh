...
# 1. Генерируем данные (input + golden)
echo "[INFO] Generating input and golden data..."
python3 scripts/gen_data.py

# 2. Запускаем C++ (читает input -> считает -> пишет output_z.bin)
(
    export LD_LIBRARY_PATH=$(pwd)/out/lib:$(pwd)/out/lib64:$LD_LIBRARY_PATH
    if [[ "$RUN_WITH_TOOLCHAIN" -eq 1 ]]; then
        # ... (логика msprof без изменений)
...
# Удаление логов симулятора
if [ "${RUN_MODE}" = "sim" ]; then
    rm -f *.log *.dump *.vcd *.toml *_log
fi

# 3. Верификация (сравнивает output_z.bin от C++ и golden.bin от Python)
echo "[INFO] Verifying results..."
# md5sum опционален, для отладки
md5sum output/*.bin
# Аргумент 1: Что насчитала программа (C++)
# Аргумент 2: Эталон (Python)
