 ...
 */
#include "config.h"  // ДОБАВЛЕНО: Используем константы из config.h
...
MatmulLeakyKernel<aType, bType, cType, biasType>::LeakyReluCompute() {
  // ДОБАВЛЕНО: Используем LEAKY_RELU_ALPHA из config.h вместо жестко заданного
  // значения 0.001
  LeakyRelu(reluOutLocal, reluOutLocal, (cType)LEAKY_RELU_ALPHA,
